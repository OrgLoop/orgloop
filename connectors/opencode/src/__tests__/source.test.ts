...
const TEST_SECRET = 'test-opencode-webhook-secret';
...
describe('OpenCodeSource', () => {
	it('initializes without error', async () => {
		...
		expect(source.id).toBe('opencode');
	});
	...
	it('receives webhook events and returns them on poll', async () => {
		...
		expect(events[0].provenance.platform).toBe('opencode');
		expect(events[0].payload.session_id).toBe('oc-sess-123');
	...
	it('cleans up on shutdown', async () => {
		...
		const req = createMockRequest(
			JSON.stringify({ session_id: 'oc-sess-456', cwd: '/tmp', exit_status: 0 }),
		);
		const res = createMockResponse();
...
describe('OpenCodeSource HMAC validation', () => {
...
describe('OpenCodeSource buffer persistence', () => {
	let bufferDir: string;

	beforeEach(() => {
		bufferDir = join(
			tmpdir(),
			`orgloop-opencode-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	it('persists events to JSONL file on disk', async () => {
		...
		const bufferPath = join(bufferDir, 'opencode-test-src.jsonl');
		...
		expect(persisted.payload.session_id).toBe('oc-sess-123');
	});

	it('survives crash — new instance reads buffered events', async () => {
		const source1 = new OpenCodeSource();
		await source1.init({
			id: 'test-src',
			connector: '@orgloop/connector-opencode',
			config: { buffer_dir: bufferDir },
		...
		const source2 = new OpenCodeSource();
		await source2.init({
			id: 'test-src',
			connector: '@orgloop/connector-opencode',
			config: { buffer_dir: bufferDir },
		...
		expect(result.events).toHaveLength(1);
		expect(result.events[0].payload.session_id).toBe('oc-sess-123');
	});
...
describe('OpenCodeSource lifecycle contract', () => {
	let source: OpenCodeSource;

	beforeEach(async () => {
		source = new OpenCodeSource();
		await source.init({
			id: 'opencode',
			connector: '@orgloop/connector-opencode',
			config: {},
	...
	it('stop hook (exit 0) emits completed/success lifecycle event', async () => {
		...
		expect(lc.dedupe_key).toBe('opencode:sess-ok:completed');
	});
	...
	it('start hook emits started lifecycle event (resource.changed)', async () => {
		...
		expect(lc.dedupe_key).toBe('opencode:sess-new:started');
	...
	it('preserves backward-compatible payload fields alongside lifecycle', async () => {
		...
		expect(event.payload.session_id).toBe('oc-sess-123');
	...
	it('session.harness is always opencode', async () => {
		const handler = source.webhook();
		const req = createMockRequest(JSON.stringify(samplePayload));
		const res = createMockResponse();
		const [event] = await handler(req, res);
	...
	it('dedupe_key is unique per session + phase', async () => {
		...
		expect(startKey).toBe('opencode:sess-x:started');
		expect(stopKey).toBe('opencode:sess-x:completed');
