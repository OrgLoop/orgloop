/**
 * OpenCode source connector — hook-based (webhook receiver).
 *
 * Receives POST requests from OpenCode session hook scripts and emits
 ...
 */
function resolveLifecycle(payload: OpenCodeSessionPayload): {
...
export class OpenCodeSource implements SourceConnector {
	...
	async init(config: SourceConfig): Promise<void> {
		...
		const cfg = config.config as unknown as OpenCodeSourceConfig;
	...
	webhook(): WebhookHandler {
		return async (req: IncomingMessage, res: ServerResponse): Promise<OrgLoopEvent[]> => {
			...
			try {
				const payload = JSON.parse(body) as OpenCodeSessionPayload;
				...
				const event = buildEvent({
					...
					payload: {
						lifecycle: {
							...
							...(reason ? { reason } : {}),
							dedupe_key: buildDedupeKey('opencode', sessionId, phase),
						},
						session: {
							id: sessionId,
							adapter: 'opencode',
