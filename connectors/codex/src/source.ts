/**
 * Codex source connector — hook-based (webhook receiver).
 *
 * Receives POST requests from Codex session hook scripts and emits
 ...
 */
function resolveLifecycle(payload: CodexSessionPayload): {
...
export class CodexSource implements SourceConnector {
	readonly id = 'codex';
	private sourceId = 'codex';
	...
	async init(config: SourceConfig): Promise<void> {
		...
		const cfg = config.config as unknown as CodexSourceConfig;
		...
		if (cfg.buffer_dir) {
			...
			this.bufferPath = join(dir, `codex-${this.sourceId}.jsonl`);
	...
	webhook(): WebhookHandler {
		return async (req: IncomingMessage, res: ServerResponse): Promise<OrgLoopEvent[]> => {
			...
			try {
				const payload = JSON.parse(body) as CodexSessionPayload;
				...
				const event = buildEvent({
					...
					payload: {
						lifecycle: {
							...
							...(reason ? { reason } : {}),
							dedupe_key: buildDedupeKey('codex', sessionId, phase),
						},
						session: {
							id: sessionId,
							adapter: 'codex',
