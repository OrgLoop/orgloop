/**
 * Pi-rust source connector — hook-based (webhook receiver).
 *
 * Receives POST requests from Pi-rust session hook scripts and emits
...
interface PiRustSessionPayload {
	session_id: string;
	cwd?: string;
 ...
 */
function resolveLifecycle(payload: PiRustSessionPayload): {
...
export class PiRustSource implements SourceConnector {
	readonly id = 'pi-rust';
	private sourceId = 'pi-rust';
	...
	async init(config: SourceConfig): Promise<void> {
		...
		const cfg = config.config as unknown as PiRustSourceConfig;
		...
		if (cfg.buffer_dir) {
			...
			this.bufferPath = join(dir, `pi-rust-${this.sourceId}.jsonl`);
	...
	webhook(): WebhookHandler {
		return async (req: IncomingMessage, res: ServerResponse): Promise<OrgLoopEvent[]> => {
			...
			try {
				const payload = JSON.parse(body) as PiRustSessionPayload;
				...
				const event = buildEvent({
					...
					payload: {
						lifecycle: {
							...
							...(reason ? { reason } : {}),
							dedupe_key: buildDedupeKey('pi-rust', sessionId, phase),
						},
						session: {
							id: sessionId,
							adapter: 'pi-rust',
							harness: 'pi-rust' as const,
							cwd: cwd || undefined,
