...
const TEST_SECRET = 'test-pi-rust-webhook-secret';
...
describe('PiRustSource', () => {
	it('initializes without error', async () => {
		const source = new PiRustSource();
		await source.init({
			id: 'pi-rust',
			connector: '@orgloop/connector-pi-rust',
			config: {},
		});
		expect(source.id).toBe('pi-rust');
	});
	...
	it('receives webhook events and returns them on poll', async () => {
		...
		expect(events[0].source).toBe('pi-rust');
		expect(events[0].provenance.platform).toBe('pi-rust');
		expect(events[0].payload.session_id).toBe('pir-sess-123');
	...
	it('cleans up on shutdown', async () => {
		...
		const req = createMockRequest(
			JSON.stringify({ session_id: 'pir-sess-456', cwd: '/tmp', exit_status: 0 }),
		);
		const res = createMockResponse();
...
describe('PiRustSource HMAC validation', () => {
...
describe('PiRustSource buffer persistence', () => {
	let bufferDir: string;

	beforeEach(() => {
		bufferDir = join(
			tmpdir(),
			`orgloop-pi-rust-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	it('persists events to JSONL file on disk', async () => {
		...
		const bufferPath = join(bufferDir, 'pi-rust-test-src.jsonl');
		...
		expect(persisted.payload.session_id).toBe('pir-sess-123');
	});

	it('survives crash — new instance reads buffered events', async () => {
		const source1 = new PiRustSource();
		await source1.init({
			id: 'test-src',
			connector: '@orgloop/connector-pi-rust',
			config: { buffer_dir: bufferDir },
		...
		const source2 = new PiRustSource();
		await source2.init({
			id: 'test-src',
			connector: '@orgloop/connector-pi-rust',
			config: { buffer_dir: bufferDir },
		...
		expect(result.events).toHaveLength(1);
		expect(result.events[0].payload.session_id).toBe('pir-sess-123');
	});
...
describe('PiRustSource lifecycle contract', () => {
	let source: PiRustSource;

	beforeEach(async () => {
		source = new PiRustSource();
		await source.init({
			id: 'pi-rust',
			connector: '@orgloop/connector-pi-rust',
			config: {},
	...
	it('stop hook (exit 0) emits completed/success lifecycle event', async () => {
		...
		expect(lc.dedupe_key).toBe('pi-rust:sess-ok:completed');
	});
	...
	it('preserves backward-compatible payload fields alongside lifecycle', async () => {
		...
		expect(event.payload.session_id).toBe('pir-sess-123');
		expect(event.payload.cwd).toBe('/tmp/test-project');
		expect(event.payload.duration_seconds).toBe(75);
		expect(event.payload.exit_status).toBe(0);
		expect(event.payload.summary).toBe('Refactored auth module');
		expect(event.payload.model).toBe('pi-rust-1');
	});

	it('session.harness is always pi-rust', async () => {
		const handler = source.webhook();
		const req = createMockRequest(JSON.stringify(samplePayload));
		const res = createMockResponse();
		const [event] = await handler(req, res);
	...
	it('dedupe_key is unique per session + phase', async () => {
		...
		expect(startKey).toBe('pi-rust:sess-x:started');
		expect(stopKey).toBe('pi-rust:sess-x:completed');
