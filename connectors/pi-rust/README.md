# @orgloop/connector-pi-rust

Captures Pi-rust session lifecycle events via a webhook handler. This connector exposes an HTTP endpoint that receives POST requests from Pi-rust hook scripts (start + stop).

## Install

```bash
npm install @orgloop/connector-pi-rust
```
...
sources:
  - id: pi-rust
    ...
    config:
      secret: "${PI_RUST_WEBHOOK_SECRET}"      # optional — HMAC-SHA256 validation
      buffer_dir: "/tmp/orgloop-pi-rust"       # optional — persist events to disk
    poll:
...
|---|---|---|
| `session.started` | Pi-rust session starts | Session launched (start hook) |
| `session.completed` | Pi-rust session ends with exit 0 | Session completed successfully |
| `session.failed` | Pi-rust session ends with non-zero exit | Session failed |
| `session.stopped` | Pi-rust session ends via signal | Session stopped/cancelled |
...
```json
{
  ...
  "payload": {
    "lifecycle": {
      ...
      "outcome": "success",
      "reason": "exit_code_0",
      "dedupe_key": "pi-rust:pir-sess-abc123:completed"
    },
    "session": {
      "id": "pir-sess-abc123",
      "adapter": "pi-rust",
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `session_id` | `string` | yes | Pi-rust session identifier |
...
routes:
  - name: pi-rust-exit-review
    ...
    then:
      ...
      config:
        session_key: "orgloop:pi-rust:session-review"
    with:
      prompt_file: sops/review-pi-rust-session.md
```

## Auth / prerequisites

- **No API tokens needed** — this connector receives events via HTTP webhook.
- A Pi-rust **Stop hook** must be configured to POST session data to the OrgLoop webhook endpoint when a session exits.
...
|----------|----------|-------------|
| `PI_RUST_WEBHOOK_SECRET` | No | HMAC-SHA256 secret for validating incoming webhook signatures |
