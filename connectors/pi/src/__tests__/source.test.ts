...
const TEST_SECRET = 'test-pi-webhook-secret';
...
describe('PiSource', () => {
	...
	it('receives webhook events and returns them on poll', async () => {
		...
		expect(events[0].provenance.platform).toBe('pi');
		expect(events[0].payload.session_id).toBe('pi-sess-123');
	...
	it('cleans up on shutdown', async () => {
		...
		const req = createMockRequest(
			JSON.stringify({ session_id: 'pi-sess-456', cwd: '/tmp', exit_status: 0 }),
		);
		const res = createMockResponse();
...
describe('PiSource HMAC validation', () => {
...
describe('PiSource buffer persistence', () => {
	let bufferDir: string;

	beforeEach(() => {
		bufferDir = join(
			tmpdir(),
			`orgloop-pi-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
		);
	...
	it('persists events to JSONL file on disk', async () => {
		...
		const bufferPath = join(bufferDir, 'pi-test-src.jsonl');
		...
		expect(persisted.payload.session_id).toBe('pi-sess-123');
	});

	it('survives crash — new instance reads buffered events', async () => {
		const source1 = new PiSource();
		await source1.init({
			id: 'test-src',
			connector: '@orgloop/connector-pi',
			config: { buffer_dir: bufferDir },
		...
		expect(result.events).toHaveLength(1);
		expect(result.events[0].payload.session_id).toBe('pi-sess-123');
	});
...
describe('PiSource lifecycle contract', () => {
	let source: PiSource;
	...
	it('stop hook (exit 0) emits completed/success lifecycle event', async () => {
		...
		expect(lc.dedupe_key).toBe('pi:sess-ok:completed');
	});
	...
	it('preserves backward-compatible payload fields alongside lifecycle', async () => {
		...
		expect(event.payload.session_id).toBe('pi-sess-123');
		...
		expect(event.payload.model).toBe('pi-1');
	});

	it('session.harness is always pi', async () => {
		const handler = source.webhook();
		const req = createMockRequest(JSON.stringify(samplePayload));
		const res = createMockResponse();
		const [event] = await handler(req, res);
	...
	it('dedupe_key is unique per session + phase', async () => {
		...
		expect(startKey).toBe('pi:sess-x:started');
		expect(stopKey).toBe('pi:sess-x:completed');
