...
|---|---|---|
| `session.started` | Pi session starts | Session launched (start hook) |
| `session.completed` | Pi session ends with exit 0 | Session completed successfully |
| `session.failed` | Pi session ends with non-zero exit | Session failed |
| `session.stopped` | Pi session ends via signal | Session stopped/cancelled |
...
| Field | Type | Required | Description |
|-------|------|----------|-------------|
| `session_id` | `string` | yes | Pi session identifier |
...
|----------|----------|-------------|
| `PI_WEBHOOK_SECRET` | No | HMAC-SHA256 secret for validating incoming webhook signatures |
