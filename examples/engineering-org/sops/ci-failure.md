...
A CI workflow run has failed. Your job is to diagnose and fix it.

## Instructions

1. Read the workflow run details — which job failed, which step
2. Pull the failing branch and reproduce locally if possible
3. **Test failures** -> read the failing test, understand what it expects, fix the code or the test
