...
- CLI: `init` (with `--template`), `validate`, `plan`, `apply`, `stop`, `status`, `logs`, `test`
...
- Templates: `orgloop init --template <name>` with 2-3 built-in templates

**Out of scope for MVP:**
- Modules (v1.1+ — see [Section 12](#12-modules-v11))
...
├── deliver.attempt    engineering-agent                   evt_001   t=19ms
└── deliver.success    engineering-agent                   evt_001   t=108ms
```
